...
评测 **AI 生成的 PPT 文件**（Kimi / Gamma / 讯飞智文 / WPS AI 等工具的产出），同样采用三层十维体系：

| 层 | 维度 | 打分通路 |
|---|---|---|
| 内容层 | P01 内容完整性 / P02 逻辑连贯性 / P03 文本质量 / P04 信息真实性（AI 幻觉） | 大模型读取全文大纲 |
| 设计层 | P05 排版质量 / P06 配色可读性 / P07 风格一致性 | 幻灯片渲染成图 + 大模型视觉评分 |
| 语义层 | P08 主题符合度 / P09 受众与场景适配 | 大模型（对照生成要求） |
| 本地指标 | P10 结构规范度（密度/字号/字体/溢出） | python-pptx 本地启发式，零成本 |

设计层维度依赖幻灯片渲染：本机装有 **PowerPoint**（COM 导出，经隔离子进程）或 **LibreOffice** 即自动启用；两者都没有时这些维度优雅降级为不可用，内容类维度不受影响。
