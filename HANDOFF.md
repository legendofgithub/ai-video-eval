...
> 更新时间：2026-09-09 16:20 +08:00
> 当前结论：P1/P2/P3 已完成；P4 的主观评分、专家仲裁、可靠性、VBench 导出、托盘与边界测试、产品介绍页已进入网页端。2026-09-07 工作台播放器、2026-09-09 一键测评/测评档案/启动容错/数据目录收拢已全部提交为 `7aa1c56`。工作区干净。
> 当前 Git：`master`；最近提交为 `7aa1c56`（feat: 工作台播放器 + 一键测评归档；启动容错与数据目录收拢），其前为 `1436538`、`e4c0891`。桌面 exe（123.5MB）已含上述全部能力。
> 当前运行状态：用户桌面实例正常运行于 8765；`data/` 内含用户真实数据（Key、上传视频），永远不在清理范围。`8876/8904` 为外部进程，未擅停。
...
10. **PPT 评测模块 v1（2026-09-09 晚）**：`core/ppt.py` 新增 PPT 维度体系 P01–P10（内容层 P01–P04 / 设计层 P05–P07 / 语义层 P08–P09 / 本地指标 P10 结构规范度），`spec_ppt_default_10`。存储新增 `decks`/`deck_scores` 表与 `data/decks/` 目录（file_hash 去重、id 正则防穿越）。渲染走**优雅降级**：PowerPoint COM（经 `--render-child` 隔离子进程，导出后文件名归一化 slide-NN.png）优先，LibreOffice+PDF 兜底，两者皆无时设计维度 `NO_RENDERER` 且不落库；渲染器检查先于 LLM 配置检查（避免修复提示被掩盖）。新路由 `/api/ppt/dimensions|renderer`、`/api/deck/upload|evaluate/{dim}|evaluate-all|scores|dashboard|slide/{n}.png`。前端新增「PPT 评测」页（上传/维度卡带需渲染器+模型角标/一键测评/幻灯片预览网格/工具子榜）。依赖新增 python-pptx 1.0.2、pywin32 312（requirements+lock）。真实验证：COM 渲染 5 页 PNG 成功、P10=6.0 正确抓到 10pt 小字号、LMM 链路通到 DeepSeek 但**用户桌面保存的 Key 已被官方判定失效（401，尾号 e620）**——需用户在应用内「更换」重输新 Key；502 失败不落库已验证。测试 63 passed/2 skipped（+8 PPT 用例），ruff/mypy（11 文件）/node 全绿。仓库已转 public 并新增根 README；后续提交直接 push origin main。
